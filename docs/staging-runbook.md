...
# .env의 DATABASE_URL을 Neon direct URL로 설정
corepack pnpm db:migrate
corepack pnpm db:seed
# 이후 실행 시에는 pooled URL로 DATABASE_URL을 교체
```
...
2026-09-16 기준 Neon direct 연결로 migration과 seed를 적용했습니다. API가 저장소 루트 `.env`를 자동으로 로드하며, `USE_IN_MEMORY_DB=false` 상태에서 전체 사용자 흐름과 프로세스 재시작 후 결과·아카이브 영속성을 검증했습니다. 실제 서비스 실행 전에는 `DATABASE_URL`을 Neon pooled URL로 교체합니다.
